...
<template>
    <div class="videoBox wrap" id="wrap">
        <div class="videoTitle">
            ...
            <span class="btn" @click="openScreen">开启投屏</span>
        </div>
        <div class="videoWrap">
                    ...
                    <div class="treeBox">
                        <dl v-for="(item,index) in list" :key="index" :class="{showDD:idxDD==index}">
                            ...
                            <dd v-for="(i,idx) in item.Nodes" :key="idx" @click="linkVideo(i,idx)" :class="[{now:idxLink==idx},{gray:i.Status==2}]">
                                ...
                                <span v-if="i.windowName">（{{i.windowName}} 号屏）</span>
                            </dd>
                        </dl>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        init () {
            ...
            let path2 = path1.join(__dirname, '../../../../../static/Debug/DHVideoClient.exe')
            let cmdPath = path.replace(/\\node_modules\\electron\\dist\\electron.exe/, '') + '\\' + path2
...
.rightVideoBox{
    height: 100%; display: flex; justify-content: flex-start; align-items: flex-start; overflow-y: auto;
    >ul{
        display: flex; justify-content: flex-start; align-items: flex-start; width: 100%; flex-wrap: wrap;
        >li{
            box-sizing: border-box; overflow: hidden; display: flex; flex-direction: column; justify-content: center; align-items: center; padding-left: 15px; width: 25%; padding-bottom: 15px; height: 240px;
            >div{
    ...
    .searchBox{
        ...
        .treeBox{
            ...
            dd{
                ...
                span:first-child{
                    width: 109px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
                }
                .iconFont{
                    font-style: normal;
                }
